"""Foreign/Relocation contracts: actual local facts, mocked Luna, real PDFs."""
import copy
...
from modules.focused_reports import foreign_evidence as collector, dispatcher, pdf_adapter
...
class ForeignTests(unittest.TestCase):
    ...
    def test_exact_keys_count_and_fail_closed(self):
        self.assertEqual(set(FOREIGN_HORIZONS), {q.question_key for q in QUESTIONS if q.category == "foreign"})
        self.assertEqual(len(FOREIGN_HORIZONS), 6)
    ...
    def test_bilingual_specs_evidence_horizons_and_question_specific_houses(self):
        ...
        for key, months in FOREIGN_HORIZONS.items():
            spec = get_prompt_spec(key)
            objectives.add(spec.analysis_objective)
            for lang in ("en", "hi"):
                built = build_foreign_report_prompt(key, self.kundali, lang)
                prompts.add(built.prompt)
                expected = {"birth_chart_summary", "house_lord_summary", "Jupiter", "Saturn", "Rahu",
                ...
                collector.build_promotion_transit_evidence.assert_called_with(self.kundali["lagna_sign"],
                    horizon_months=months, selected_planets=("Jupiter", "Saturn", "Rahu"))
                houses = tuple(map(int, re.findall(r"House (\d+) \(", built.evidence.sections["house_lord_summary"])))
                self.assertEqual(houses, FOREIGN_HOUSES[key])
    ...
    def test_semantic_distinctions_and_boundaries(self):
        self.assertIn("city or country", get_prompt_spec("relocation_timing").analysis_objective)
        self.assertIn("without assuming relocation must be international", get_prompt_spec("relocation_timing").output_emphasis)
        self.assertIn("education, higher learning", get_prompt_spec("study_abroad").astrological_focus)
        self.assertIn("career/service", get_prompt_spec("foreign_work_timing").astrological_focus)
        self.assertIn("permanent blockage", " ".join(get_prompt_spec("foreign_plans_delay_reason").boundaries))
        for key in FOREIGN_HORIZONS:
            boundary = " ".join(get_prompt_spec(key).boundaries)
            for text in ("visa", "immigration approval", "citizenship", "residency", "university admission", "foreign job", "relocation", "settlement", "No legal/immigration advice", "resign, migrate, invest or relocate", "no outcome probabilities or arbitrary scores"):
    ...
    def test_missing_evidence_fails_before_ai(self):
        with patch("modules.payments.report_ai_client.generate_report_completion") as ai:
            ...
            for data in (None, {}, {"lagna_sign": "Libra", "planets": []}):
                with self.assertRaises(MissingEvidenceError):
                    dispatcher.generate_focused_report("study_abroad", data)
            ai.assert_not_called()

    def test_unsupported_spec_and_person_mode_fail_closed(self):
        spec = get_prompt_spec("study_abroad")
        ...
        for change, error in (({"person_mode": "dual"}, PromptAssemblyError),
            ...
            with patch.object(collector, "get_prompt_spec", return_value=replace(spec, **change)):
                with self.assertRaises(error):
                    collector.collect_foreign_evidence("study_abroad", self.kundali)
        ...
        with tempfile.TemporaryDirectory() as folder:
            for key in FOREIGN_HORIZONS:
                for lang in ("en", "hi"):
                    ...
                    for internal in (key, "foreign_move_timing", "===META===", "horizon_months", "gpt-5.6-luna", "2026-10-15"):
