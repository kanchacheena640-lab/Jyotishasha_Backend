# modules/finance/finance_generator.py

"""
FinanceGenerator -- the third concrete ReportGenerator, built to the
exact same shape as modules/love/love_generator.py and
modules/career/career_generator.py (the reference implementations for
every segment generator: Love, Career, Finance, Health, Family, Alerts).
Neither LoveGenerator nor CareerGenerator is modified by this file.
...
shared with LoveGenerator/CareerGenerator by design -- neither of those
files was modified to extract a shared helper, so this class carries its
own copy, exactly mirroring the reference implementations' shape rather
than introducing a new shared abstraction.
"""
...
from services.ai_prediction_lab.current_finance_timing_context import build_current_finance_timing_context
from services.ai_prediction_lab.current_finance_timing_prompt_builder import build_current_finance_timing_prompt
...
class FinanceGenerator(BaseAIGenerator):
    GENERATOR_VERSION = "finance_generator_v1"
    ...
    # universal report_types LOVE/CAREER use (DNA / CURRENT_PHASE /
    # DAILY_INSIGHT) -- see this phase's implementation report for how
    # they map onto "Financial DNA" / "Current Financial Phase (incl.
    # opportunities, challenges, recommended focus)" / "Practical Action
    # Guidance".
    _PROMPT_VERSIONS = {
        "DNA": "finance_profile_v1",
        "CURRENT_PHASE": "current_finance_phase_v1",
        "DAILY_INSIGHT": "finance_action_guidance_v1",
        # CURRENT_TIMING -- third layer (DNA -> CURRENT_PHASE ->
        # CURRENT_TIMING). Purely additive; DNA/CURRENT_PHASE entries
        # above are unchanged.
        "CURRENT_TIMING": "current_finance_timing_v1",
    }

    def __init__(self, *args, repository: Optional[ReportCacheRepository] = None, **kwargs):
        super().__init__(*args, **kwargs)
        # Read-only use: fetching already-cached upstream FINANCE reports
    ...
    def build_context(
        ...
        *,
        ...
        # calculate_full_kundali() and build_finance_profile_context()
        # are the existing backend/Lab functions -- user_id=None (guest
        # mode) so this generator introduces no new write to
        # UserDashaTimeline or any other existing table, same as
        # LoveGenerator/CareerGenerator.
        ...
        finance_summary = build_finance_profile_context(kundali)  # existing Lab context builder
        ...
        if report_type == "CURRENT_PHASE":
            financial_dna_text = self._read_upstream_text(
                profile_id=profile_id, report_type="DNA", language=language,
            )
            phase_context = build_current_finance_phase_context(kundali, finance_summary)  # existing Lab context builder
            return {
                "birth_details": birth_details,
                "financial_dna": financial_dna_text,
        ...
        if report_type == "DAILY_INSIGHT":
            ...
            current_finance_phase_text = self._read_upstream_text(
                profile_id=profile_id, report_type="CURRENT_PHASE", language=language,
            )
            finance_action_ctx = build_finance_action_context(kundali)  # existing Lab context builder
            return {
                "financial_dna": financial_dna_text,
                "current_finance_phase": current_finance_phase_text,
                "finance_action_context": finance_action_ctx,
            }
        ...
        if report_type == "CURRENT_TIMING":
            ...
            timing_context = build_current_finance_timing_context(kundali)  # existing Lab context builder
            return {
                "current_finance_phase": current_finance_phase_text,
                "timing_context": timing_context,
            }

        raise ContextBuildError(f"FinanceGenerator does not support report_type={report_type!r}")
    ...
    def build_prompt(
        ...
        *,
        ...
        if report_type == "CURRENT_PHASE":
            ...
            return build_current_finance_phase_prompt(
        ...
        if report_type == "DAILY_INSIGHT":
            return build_finance_action_guidance_prompt(
                financial_dna=context["financial_dna"],
        ...
        if report_type == "CURRENT_TIMING":
            return build_current_finance_timing_prompt(
                current_finance_phase=context["current_finance_phase"],
                context=context["timing_context"],
                language=language,
            )  # existing Lab prompt builder

        raise PromptBuildError(f"FinanceGenerator does not support report_type={report_type!r}")
    ...
    # -- same as LoveGenerator/CareerGenerator.
    # ------------------------------------------------------------
    ...
    # not shared -- see module docstring) -- these do not duplicate any
    # existing logic, they only route to it.
    # ------------------------------------------------------------
    ...
    def _read_upstream_text(self, *, profile_id: int, report_type: str, language: str) -> str:
        """Read-only lookup of an already-cached FINANCE report's text
        via the existing, generic ReportCacheRepository. Raises if that
        upstream report hasn't been generated yet -- see
        LoveGenerator's/CareerGenerator's identical method for why
        sequencing (DNA before CURRENT_PHASE before DAILY_INSIGHT) is
        assumed rather than auto-triggered here."""
        ...
        if row is None or row.status != "READY" or not row.content_json:
            raise ContextBuildError(
                f"FINANCE {report_type} must be generated before it can be used as "
