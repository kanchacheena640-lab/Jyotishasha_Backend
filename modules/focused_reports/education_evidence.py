"""Minimum SELF Education evidence, using existing astrology primitives only."""
...
def collect_education_evidence(question_key, kundali, language="en"):
    spec = get_prompt_spec(question_key)
    if question_key not in EDUCATION_HORIZONS:
        raise ValueError("Question has no Education evidence collector.")
    if spec.person_mode != "single" or spec.intent_slug != "study_exam_timing":
        raise PromptAssemblyError("Education evidence requires the customer's own single chart.")
    ...
    houses = EDUCATION_HOUSES[question_key]
    ...
    transits = build_promotion_transit_evidence(kundali["lagna_sign"],
        horizon_months=EDUCATION_HORIZONS[question_key], selected_planets=TRANSIT_PLANETS)
    ...
    return EducationEvidence(lang, sections, transits, transits["as_of"], transits["horizon_end"],
