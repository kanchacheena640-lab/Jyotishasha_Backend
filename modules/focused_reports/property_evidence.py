"""Minimum SELF Property evidence, using existing astrology primitives only."""
...
def collect_property_evidence(question_key, kundali, language="en"):
    spec = get_prompt_spec(question_key)
    if question_key not in PROPERTY_HORIZONS:
        raise ValueError("Question has no Property evidence collector.")
    if spec.person_mode != "single" or spec.intent_slug != "property_timing":
        raise PromptAssemblyError("Property evidence requires the customer's own single chart.")
    ...
    houses = PROPERTY_HOUSES[question_key]
    ...
    transits = build_promotion_transit_evidence(kundali["lagna_sign"],
        horizon_months=PROPERTY_HORIZONS[question_key], selected_planets=TRANSIT_PLANETS)
    ...
    return PropertyEvidence(lang, sections, transits, transits["as_of"], transits["horizon_end"],
