...
test_health.py
--------------
Local-only entry point for the AI Prediction Lab's Health Profile.
Mirrors test_love.py's/test_career.py's/test_finance.py's exact
structure and workflow for the HEALTH segment.
...
It never generates, and must never be read as, medical advice.
"""
...
# ----------------------------------------------------------------------
# Same demo birth data test_love.py/test_career.py/test_finance.py use
# (Ravi, Lucknow, Uttar Pradesh, India) -- so all four segments' outputs
# for the same chart are directly comparable during manual review.
# ----------------------------------------------------------------------
DEMO_PERSON = {
    "name": "Ravi",
    "dob": "1985-03-31",
...
OUTPUT_ROOT = os.path.join(
    os.path.dirname(__file__), "services", "ai_prediction_lab", "output", "health_profile"
)
...
def main():
    ...
    # 2) Build the Health Profile context.
    print("-> Building Health Profile context...")
    context = build_health_profile_context(kundali)
    print("[OK] Health Profile context built.")
    ...
    prompt = build_health_profile_prompt(context)
    ...
    print("\n" + "=" * 60)
    print("AI HEALTH PROFILE RESPONSE (SECTION 1: HEALTH DNA)")
    print("=" * 60)
    print(response_text)
    print("=" * 60)

    # ------------------------------------------------------------------
    # SECTION 2: CURRENT HEALTH PHASE (builds on Section 1's response)
    # ------------------------------------------------------------------
    print("\n-> Building Current Health Phase context...")
    phase_context = build_current_health_phase_context(kundali, context)
    print("[OK] Current Health Phase context built.")

    phase_context_path = os.path.join(run_dir, "health_phase_context.json")
    with open(phase_context_path, "w", encoding="utf-8") as f:
        json.dump(phase_context, f, indent=2, ensure_ascii=False, default=str)
    print(f"[OK] health_phase_context.json saved -> {phase_context_path}")

    print("-> Loading Current Health Phase template and building final prompt...")
    phase_prompt = build_current_health_phase_prompt(
        birth_date=DEMO_PERSON["dob"],
        birth_time=DEMO_PERSON["tob"],
        birth_place=DEMO_PERSON.get("pob", "Not specified"),
        health_dna=response_text,
        context=phase_context,
    )
    print("[OK] Prompt generated.")

    phase_prompt_path = os.path.join(run_dir, "health_phase_prompt.txt")
    with open(phase_prompt_path, "w", encoding="utf-8") as f:
        f.write(phase_prompt)
    print(f"[OK] health_phase_prompt.txt saved -> {phase_prompt_path}")

    print("-> Sending prompt to OpenAI...")
    phase_response_text, phase_raw_openai = openai_client.generate_with_raw(phase_prompt)
    print("[OK] Response received.")

    phase_response_path = os.path.join(run_dir, "health_phase_response.txt")
    with open(phase_response_path, "w", encoding="utf-8") as f:
        f.write(phase_response_text)
    print(f"[OK] health_phase_response.txt saved -> {phase_response_path}")

    print("\n" + "=" * 60)
    print("AI RESPONSE (SECTION 2: CURRENT HEALTH PHASE)")
    print("=" * 60)
    print(phase_response_text)
    print("=" * 60)

    # ------------------------------------------------------------------
    # SECTION 3: PRACTICAL HEALTH GUIDANCE (builds on Sections 1 and 2)
    # ------------------------------------------------------------------
    print("\n-> Building health action context...")
    action_context = build_health_action_context(kundali)
    print("[OK] Health action context built.")

    action_context_path = os.path.join(run_dir, "health_action_context.json")
    with open(action_context_path, "w", encoding="utf-8") as f:
        json.dump(action_context, f, indent=2, ensure_ascii=False, default=str)
    print(f"[OK] health_action_context.json saved -> {action_context_path}")

    print("-> Loading Action Guidance template and building final prompt...")
    action_prompt = build_health_action_guidance_prompt(
        health_dna=response_text,
        current_health_phase=phase_response_text,
        health_action_context=action_context,
    )
    print("[OK] Prompt generated.")

    action_prompt_path = os.path.join(run_dir, "health_action_prompt.txt")
    with open(action_prompt_path, "w", encoding="utf-8") as f:
        f.write(action_prompt)
    print(f"[OK] health_action_prompt.txt saved -> {action_prompt_path}")

    print("-> Sending prompt to OpenAI...")
    action_response_text, action_raw_openai = openai_client.generate_with_raw(action_prompt)
    print("[OK] Response received.")

    action_response_path = os.path.join(run_dir, "health_action_response.txt")
    with open(action_response_path, "w", encoding="utf-8") as f:
        f.write(action_response_text)
    print(f"[OK] health_action_response.txt saved -> {action_response_path}")

    print("\n" + "=" * 60)
    print("AI RESPONSE (SECTION 3: PRACTICAL HEALTH GUIDANCE)")
    print("=" * 60)
