"""Education contracts: actual local facts, mocked Luna, real PDFs."""
import copy
...
from modules.focused_reports import education_evidence as collector, dispatcher, pdf_adapter
from modules.focused_reports.education_keys import EDUCATION_HORIZONS, EDUCATION_HOUSES
...
class EducationTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with frozen_clock():
            ...
            cls.transits = {m: collector.build_promotion_transit_evidence(
                ...
                selected_planets=collector.TRANSIT_PLANETS) for m in (12, 36)}
    ...
    def test_bilingual_specs_evidence_horizons_and_question_specific_houses(self):
        ...
        for key, months in EDUCATION_HORIZONS.items():
            spec = get_prompt_spec(key)
            objectives.add(spec.analysis_objective)
            for lang in ("en", "hi"):
                built = build_education_report_prompt(key, self.kundali, lang)
                prompts.add(built.prompt)
                expected = {"birth_chart_summary", "house_lord_summary", "Jupiter", "Saturn",
                            "dasha_sequence" if months == 36 else "dasha_window_summary"}
                ...
                self.assertEqual(built.evidence.horizon_end, "2029-09-21" if months == 36 else "2027-09-21")
                ...
                self.assertEqual(houses, EDUCATION_HOUSES[key])
    ...
    def test_semantic_distinctions_and_boundaries(self):
        self.assertIn("sustained learning", get_prompt_spec("study_strong_period").analysis_objective)
        self.assertIn("preparation habits and revision", get_prompt_spec("exam_preparation_period").output_emphasis)
        self.assertIn("higher-learning", get_prompt_spec("higher_education_timing").astrological_focus)
        self.assertIn("slow learning progress", get_prompt_spec("study_progress_improvement").analysis_objective)
        self.assertIn("not permanent blockage", " ".join(get_prompt_spec("study_progress_improvement").boundaries))
        for key in EDUCATION_HORIZONS:
            boundary = " ".join(get_prompt_spec(key).boundaries)
            for text in ("No guaranteed exam success", "pass/fail", "marks/rank", "admission", "scholarship", "institution acceptance", "visa", "degree completion", "No marks/rank prediction", "exam success percentage", "admission probability", "arbitrary education score", "Do not advise abandoning education"):
    ...
    def test_missing_evidence_fails_before_ai(self):
        with patch("modules.payments.report_ai_client.generate_report_completion") as ai:
            ...
            for data in (None, {}, {"lagna_sign": "Libra", "planets": []}):
                with self.assertRaises(MissingEvidenceError):
                    dispatcher.generate_focused_report("study_strong_period", data)
            ai.assert_not_called()

    def test_unsupported_spec_and_person_mode_fail_closed(self):
        spec = get_prompt_spec("study_strong_period")
        ...
        for change, error in (({"person_mode": "dual"}, PromptAssemblyError),
            ...
            with patch.object(collector, "get_prompt_spec", return_value=replace(spec, **change)):
                with self.assertRaises(error):
                    collector.collect_education_evidence("study_strong_period", self.kundali)
        ...
        with tempfile.TemporaryDirectory() as folder:
            for key in EDUCATION_HORIZONS:
                for lang in ("en", "hi"):
                    ...
                    self.assertIsNone(args["disclaimer"])  # No unrelated career/financial policy.
                    ...
                    for internal in (key, "study_exam_timing", "===META===", "horizon_months", "gpt-5.6-luna", "2026-10-15"):
