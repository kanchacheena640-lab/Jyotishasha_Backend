# services/ai_prediction_lab/current_career_timing_context.py

"""
services/ai_prediction_lab/current_career_timing_context.py
-------------------------------------------------------------
Builds the "Current Career Timing" AI context -- CAREER's THIRD layer
...
signature (CareerGenerator's call site stays unchanged).
...
services.ai_prediction_lab.career_context_builder.build_career_profile_context()
in place of the Love equivalent).

Tracks Sun, Mercury, Moon -- CAREER's already-established
"fast-moving, for Current Timing" planet set (unchanged from before;
current_career_phase_v1.txt already labels exactly these three planets
this way) -- plus which other planets currently share Moon's sign, and
the natal 10th Lord's CURRENT transit position ("foundation").
"""
...
CAREER_TIMING_PLANETS = ["Sun", "Mercury", "Moon"]
...
def build_current_career_timing_context(kundali: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    Returns CAREER's fast-moving current transits (Sun, Mercury, Moon),
    Moon's current conjunction planets (if any), and the natal 10th
    Lord's CURRENT transit position (sign/house only) -- no Mahadasha,
    no Antardasha, no career_phase, no other permanent chart facts.
    """
    ...
    result: Dict[str, Any] = {
        ...
        for planet in CAREER_TIMING_PLANETS
    }
    result["moon_conjunctions"] = _conjunct_planets("Moon", positions)

    career_summary = build_career_profile_context(kundali)
    tenth_lord_name = (career_summary.get("tenth_lord") or {}).get("name")
    result["foundation_lord"] = (
        _timing_transit_summary(tenth_lord_name, lagna_sign, positions)
        if tenth_lord_name else {"sign": None, "house": None, "nakshatra": None, "motion": None}
    )
