"""
services/ai_prediction_lab/career_action_context.py
-------------------------------------------------------------
Foundation for the Career segment's third layer (Action Guidance) -- a
dedicated, isolated context object carrying ONLY the fast-moving current
transits relevant to short-term career action (Moon, Mercury, Sun, Mars).
Mirrors daily_transit_context.py's exact pattern for the LOVE segment.

Deliberately kept separate from current_career_phase_context.py (slower
...
module has no import dependency on current_career_phase_context.py at
...
# Moon and Mercury are the same fast movers LOVE's daily layer already
# uses; Sun replaces Venus here (not a career significator) since Sun is
# an explicit career factor for this segment and moves at a comparable
# daily pace. Mars is kept for its shared relevance to drive/initiative.
CAREER_ACTION_PLANETS = ["Moon", "Mercury", "Sun", "Mars"]
...
def build_career_action_context(kundali: Dict[str, Any]) -> Dict[str, Any]:
    """
