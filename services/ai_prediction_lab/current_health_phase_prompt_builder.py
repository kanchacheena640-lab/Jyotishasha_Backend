"""
services/ai_prediction_lab/current_health_phase_prompt_builder.py
--------------------------------------------------------------------
Loads prompts/current_health_phase_v1.txt and fills it with the
Current Health Phase context (current_health_phase_context.py) plus the
birth details and the already-generated Health DNA text (first
section). Mirrors current_finance_phase_prompt_builder.py's exact
pattern.
...
_CURRENT_HEALTH_PHASE_TEMPLATE = os.path.join(_TEMPLATE_DIR, "current_health_phase_v1.txt")

_TRANSIT_PLANETS = ["moon", "sun", "mars", "saturn", "jupiter"]
...
def build_current_health_phase_prompt(
    birth_date: str,
    birth_time: str,
    birth_place: str,
    health_dna: str,
    ...
    template = _load_template(_CURRENT_HEALTH_PHASE_TEMPLATE)

    mahadasha = context.get("mahadasha", {})
    antardasha = context.get("antardasha", {})
    phase = context.get("health_phase", {})
    ...
    values: Dict[str, str] = {
        ...
        "health_phase_reasons": _reasons_to_text(phase.get("reasons", [])),
    }
    ...
    for key in _TRANSIT_PLANETS:
        ...
        values[f"{key}_health_role"] = t.get("health_role") or "Unknown"
