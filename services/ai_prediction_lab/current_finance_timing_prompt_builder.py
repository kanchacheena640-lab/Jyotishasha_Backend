# services/ai_prediction_lab/current_finance_timing_prompt_builder.py

"""
services/ai_prediction_lab/current_finance_timing_prompt_builder.py
--------------------------------------------------------------------
Loads prompts/current_finance_timing_v1.txt and fills it with the
Current Finance Timing context (current_finance_timing_context.py)
plus the already-generated Current Financial Phase text (the report
this layer continues from). Mirrors
current_love_timing_prompt_builder.py's exact pattern.
"""
...
_CURRENT_FINANCE_TIMING_TEMPLATE = os.path.join(_TEMPLATE_DIR, "current_finance_timing_v1.txt")

_TIMING_PLANETS = ["venus", "mercury", "moon"]
...
def build_current_finance_timing_prompt(
    current_finance_phase: str,
    context: Dict[str, Any],
    language: str = "en",
) -> str:
    template = _load_template(_CURRENT_FINANCE_TIMING_TEMPLATE)

    values: Dict[str, str] = {
        "current_phase_text": current_finance_phase,
