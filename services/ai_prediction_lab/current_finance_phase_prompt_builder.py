...
def build_current_finance_phase_prompt(
    ...
    values: Dict[str, str] = {
        ...
        # computed in current_finance_phase_context.py). Falls back to
        # the raw Antardasha end date, then "Unknown", if that
        # computation returned nothing. Feeds a DIFFERENT template
        # placeholder ({next_phase_change_date}, only in the NEXT PHASE
        # CHANGE RULES section) so it never overwrites the factual
        # Antardasha display above.
